function [ trialCov ] = riemann_cov( trial )
% Extraction of covariance matrices in barachant format ( To adjust accordingly with that of covariance toolbox)


n_ch = size(trial,2);
n_trial = size(trial,3);
nt = size(trial,1);


trialCov = zeros(n_ch,n_ch,n_trial);
for t=1:n_trial
%     tic
    E = trial(:,:,t)';
    EE = E' * E;
%     trialCov(:,:,t) = cov1para(trial(:,:,t)./ sqrt(trace(EE))); %estimation of trial covariances using automatic covariance matrix shrinking
    trialCov(:,:,t) = cov1para(trial(:,:,t));
end
