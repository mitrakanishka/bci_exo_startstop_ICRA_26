...
function [jp, rej, distribution, data2idx] = jointprob( signal, threshold, oldjp, normalize, discret );
...
if exist('oldjp') & ~isempty( oldjp ) % speed up the computation
    ...
    distribution = [];
    data2idx = cell(nbchan, 1);
	for rc = 1:nbchan

		% COMPUTE THE DENSITY FUNCTION
		% ----------------------------
		[ dataProba, temp, data2idx{rc}] = realproba(signal(rc, :), discret);
        distribution = cat(2, distribution, temp');

		% compute all entropy
		% -------------------
		for index=1:sweeps
			datatmp = dataProba((index-1)*pnts+1:index*pnts);
			jp(rc, index) = - sum( datatmp .* log( datatmp ) ); 
